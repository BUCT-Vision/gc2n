...
for (FLAG,curr_train_ratio) in [(1, 0.1)]:
    ...
    #train_samples_per_class=[15,50,50,50,50,50,15,50,15,50,50,50,50,50,50,50]
    #train_samples_per_class=[5,30,30,30,30,30,3,30,3,30,30,30,30,30,30,10]
    ...
    for curr_seed in Seed_List:
        ...
        if samples_type=='same_num':
            for i in range(class_count):
                idx = np.where(gt_reshape == i + 1)[-1]
                samplesCount = len(idx)
                real_train_samples_per_class=train_samples_per_class[i]
                rand_list = [i for i in range(samplesCount)]  # 用于随机的列表
                if real_train_samples_per_class>=samplesCount:
                    #real_train_samples_per_class=samplesCount
                    real_train_samples_per_class=int(train_samples_per_class[i]/2)
    ...
    #threshold=0.001
    #model = model_try.SSCDNonLModel_gcns(class_count, 200, 150) # Criss Cross Model CCNet 2B parallel
    #model = model_try.SSCDNonLModel_gcn6(class_count, 200, 150)
    ...
    logger = get_logger('/home/shsun/Dingwenda/ENL-FCN/Demo_IP.log')
    epoch=1800
    exp_num=10
    best=[]
    output_file=[]
    threshold=0.001
    #model_names=[model_try.SSCDNonLModel_gcn_1,model_try.SSCDNonLModel_gcn_2,model_try.SSCDNonLModel_gcn_3]

    for num in range(10):
        
        model = model_try.SSCDNonLModel_gcn_a(class_count, n_bands, 200, threshold) # Criss Cross Model CCNet 2B parallel
        #model = model_try.SSCDNonLModel_gat(class_count, n_bands, 200)
        print(model)
    ...
    #for num in range(exp_num):
        ...
        for eep in range(epoch):
            ...
            if eep%10==0:
                ...
                #dict_idx={'NUM':num,'Epoch':eep,'OA':OA,'AA':AA,'Kappa':kappa,'AC':AC.tolist()}
                #dict_all.append(dict_idx)  
                logger.info('Epoch:[{}/{}]\t test={}\{} OA={:.7f} AA={:.7f} kappa={:.7f} AC=[{}]'.format(eep , epoch, rightNum, testNum ,OA,AA,kappa,AC))
            if eep==360:
                OA=np.round(OA*100, decimals=2)
                OutputWhole = PatchStack(Output, m, n, patch_height, patch_width, split_height, split_width, EDGE, class_count+1)
                #spectral.imshow(classes=OutputWhole.astype(int),figsize=(8,8))
                Draw_Classification_Map(OutputWhole, '/home/shsun/Dingwenda/ENL-FCN/ResultImage/' + dataset_name + '_gcn_' + str(train_ratio) + '_' + str(OA))
