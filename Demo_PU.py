...
for (FLAG,curr_train_ratio) in [(2, 0.01)]:
    ...
    epoch=1200
    avg=0
    logger = get_logger('/home/shsun/Dingwenda/ENL-FCN/Demo_PU.log')
    exp_num=5
    '''
    for thre in np.arange(0.0055,0.006,0.0001):
        
        threshold=thre
        for num in range(exp_num):
            model = model_try.SSCDNonLModel_ab(class_count, n_bands, 200) # Criss Cross Model CCNet 2B parallel
        ...
        #for num in range(exp_num):
            best_acc = -1
            logger.info('NUM:[{}/{}]\t threshold={},lr={},weight+decay={} \n\n'.format(num ,exp_num, threshold,learning_rate,weight_decay))
            for eep in range(epoch):
                ...
                if eep==400:
                    ...
                    #break
            logger.info('threshold={} best_acc {:.7f} \n\n'.format(threshold,best_acc))
'''
    for num in range(10):
        model = model_try.SSCDNonLModel_gcn_a(class_count, n_bands, 200,0.001) # Criss Cross Model CCNet 2B parallel
        #print(n_bands)
        #model = model_try.SSCDNonLModel_gat(class_count, n_bands, 200)
        print(model)
        if use_cuda: torch.backends.cudnn.benchmark = True
